...
def require_admin(func):
    ...
    def wrapper(request, *args, **kwargs):
        if not request.user.is_authenticated or not request.user.is_superuser:
            return JsonResponse(
                {"success": False, "error": "Admin required"}, status=401
            )
        return func(request, *args, **kwargs)
...
def wx_login(request):
    ...
    if not APP_ID or not SECRET:
        return JsonResponse(
            {"success": False, "error": "Invalid APP_ID or SECRET"}, status=400
        )
    ...
    if response.status_code != 200:
        return JsonResponse(
            {"success": False, "error": "Login failed! res: " + response.text}
        )
    ...
    return JsonResponse(
        {
            ...
            "csrfToken": get_token(
                request
            ),  # to WeiXin mini program, make it post request with csrf token in future
...
def update_nickname(request):
    ...
    if not nickname:
        return JsonResponse(
            {"success": False, "error": "Nickname is required"}, status=400
        )
...
def do_login(request):
    ...
    if username is None or password is None:
        return JsonResponse(
            {"success": False, "error": "Invalid username or password"}, status=400
        )
    ...
    UserSession.objects.filter(
        user=user.core_user_profile, client_type="website"
    ).delete()
...
def query_review(request, id):
    ...
    if review_info is None:
        return JsonResponse({"success": False, "error": raw_dict})
...
def query_paper_info(request):
    ...
    if paper_info_old is None:
        return JsonResponse(
            {
                "success": False,
                "error": f"未找到该文献信息，请手动输入。",
            }
        )
...
def submit_review(request):
    ...
    if review is None:
        return JsonResponse(
            {"success": False, "error": f"Review not found: {review_id}"}
        )
    ...
    if (
        ...
        and review.creator != request.user.core_user_profile
    ):
        return JsonResponse(
            {
                "success": False,
...
def get_latest_deadline():
    ...
    if cci.count() > 0:
        ...
        deadline = timezone.make_aware(
            datetime.datetime(now.year, now.month, 1), timezone.get_current_timezone()
        )
    return deadline
...
def fetch_rank_full_list(request):
    ...
    if group is None:
        return JsonResponse(
            {"success": False, "error": f"Group not found: {group_name}"}
        )
...
def weixin_callback(request):
    ...
    if openid is None or unionid is None:
        return JsonResponse(
            {"success": False, "error": "Invalid openid or unionid"}, status=400
        )
    ...
    profiles = UserProfile.objects.filter(Q(wx_unionid=unionid) | Q(wx_openid=openid))
    if profiles.count() == 0:
        ...
        user.save()
        profile = UserProfile(
            auth_user=user, nickname=nickname, wx_openid=openid, wx_unionid=unionid
        )
        profile.save()
    else:
        profile = profiles[0]
...
def handle_translation(
    request, field_name, translation_func, model, translation_model, translation_field
):
    data = request.json_data
    paper_id = data.get("paper_id")
    ...
    if translation_instance and getattr(translation_instance, translation_field):
        return JsonResponse(
            {
                "success": True,
                "answer": getattr(translation_instance, translation_field),
            }
        )
    ...
    try:
        with transaction.atomic():
            translation_instance, created = (
                translation_model.objects.select_for_update().get_or_create(paper=paper)
            )
...
def check_in(request):
    ...
    if admin_user:
        user = UserProfile.objects.filter(nickname=admin_user).first()
        if not user:
            user = UserProfile(nickname=admin_user)
            user.save()
        creator = user
    else:
...
def new_remove_paper(request):
    ...
    user = request.user.core_user_profile
    review_list = Review.objects.filter(paper=paper, delete_time__isnull=True)
    if request.user.is_superuser:
        # Superuser can delete any review for this paper
        pass
    else:
        # Regular user can only delete their own reviews
        review_list = review_list.filter(creator=user)
...
def new_restore_paper(request):
    ...
    review_list = Review.objects.filter(
        paper=paper, delete_time__isnull=False, creator=user
    )
    if review_list.count() > 0:
        for review in review_list:
...
def new_edit_review(request):
    ...
    if review.creator != user:
        return JsonResponse(
            {
                "success": False,
                "error": f"Review {review_id} is not created by user {user}",
            }
        )
...
def search_users(request):
    q = request.GET.get("q", "")
    if q.strip():
        qs = UserProfile.objects.filter(nickname__icontains=q.strip())
