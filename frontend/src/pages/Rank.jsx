...
function Rank({ type: defaultType }) {
  ...
  const handleMonthChange = (e) => {
    ...
    navigate(`/group/${groupName}/rank/${rankType}?year=${year || data?.year || 2024}&month=${m}`);
  };
  ...
  return (
    <div>
      ...
      <div className="border p-3 bg-white-subtle">
        ...
        {['this_month', 'last_month', 'monthly', 'yearly'].includes(rankType) && (
        ...
        {data?.ranks && data.ranks.length > 0 ? (
          <table className="table table-bordered table-striped text-center">
            <thead>
              <tr>
                <th className="col-2">排名</th>
                <th className="col-4">{rankType === 'journal' ? '杂志' : '分享者'}</th>
                <th className="col-2">分享数</th>
                <th className="col-4">最早分享时间</th>
              </tr>
            </thead>
            <tbody>
              {data.ranks.map((row) => (
                <tr key={row.display_index}>
                  ...
                  <td>
                    {rankType === 'journal' ? (
                      <Link to={`/group/${groupName}/journal/${encodeURIComponent(row.name)}`}>
                        {row.name}
                      </Link>
                    ) : (
