  ...
  try {
    ...
    if (!response.ok) {
      const error = new Error(
        result.error || result || `HTTP ${response.status}`,
      );
...
const api = {
  ...
  checkIn: (groupName, data) =>
    request("POST", `/check-in`, { group_name: groupName, ...data }),

  // Edit & delete
  editReview: (reviewId, comment) =>
    request("POST", "/new-edit-review", { review_id: reviewId, comment }),
  editPaper: (reviewId, paperId, paperData) =>
    request("POST", "/submit-review", {
      review_id: reviewId,
      paper_id: paperId,
      ...paperData,
    }),
  removePaper: (paperId) =>
    request("POST", "/new-remove-paper", { paper_id: paperId }),
