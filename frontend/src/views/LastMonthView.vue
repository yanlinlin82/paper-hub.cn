<template>
  <div class="last-month-page">
    <div class="page-header">
      <h1 class="page-title">
        <i class="fas fa-history"></i>
        上月分享
      </h1>
      <p class="page-subtitle">{{ lastMonth }} 的论文分享统计</p>
    </div>
...
const fetchPapers = async () => {
  try {
    loading.value = true
    const data = await api.getPapers('last-month')
    papers.value = data
  } catch (error) {
    console.error('Error fetching last month papers:', error)
  } finally {
    loading.value = false
  }
}
